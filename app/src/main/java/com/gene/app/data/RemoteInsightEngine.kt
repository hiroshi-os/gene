...
object RemoteInsightEngine {
    ...
    suspend fun generatedSummary(context: Context, person: Person, memories: List<Interaction>): String = withContext(Dispatchers.IO) {
        ...
        if (endpoint == null || key.isBlank()) return@withContext InsightEngine.generatedSummary(person, memories)
        try {
            ...
            val response = rawCompletion(prefs, endpoint, key, "You are Gene's summary writer. Write exactly two concise sentences in third person about the person reconstructed from the supplied memories. Mention only supported patterns. Do not use markdown, labels, disclaimers, or phrases about being an AI. Do not claim certainty.", "Person: ${person.name}\nMemories:\n$prompt")
    ...
    private suspend fun complete(context: Context, person: Person, memories: List<Interaction>, sessionMessages: List<ChatMessage>, earlierMessages: List<ChatMessage>, question: String, personaMode: Boolean, localConfidence: Int, fallback: () -> PersonaReply): PersonaReply = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val content = rawCompletion(prefs, endpoint, key, system, conversation)
            if (content.isBlank()) fallback() else PersonaReply(compactPersonaText(content), localConfidence, memories.map { it.id })
        } catch (_: Exception) { fallback() }
    }

    private fun rawCompletion(prefs: android.content.SharedPreferences, endpoint: String, key: String, system: String, user: String): String {
        val payload = JSONObject().apply {
            put("model", prefs.getString("llm_model", "gpt-5-mini"))
