...
class AudioTranscriptionWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    override suspend fun doWork(): Result {
        ...
        val llm = LlmSettings.resolve(applicationContext)
        if (!llm.ready || llm.transcriptionEndpoint == null) {
            db.updateTranscription(interactionId, null, TRANSCRIPTION_UNAVAILABLE)
            return Result.success()
        }
        return try {
            val endpoint = llm.transcriptionEndpoint
            ...
            BufferedOutputStream(connection.outputStream).use { output ->
                ...
                part("Content-Disposition: form-data; name=\"model\"", llm.transcriptionModel)
